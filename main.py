...
def main():
    ...
    parser.add_argument("--asn", help="Comma-separated ASN list (optional)")
    parser.add_argument("--out", default="output", help="Output directory")
    parser.add_argument("--include-routeviews", action="store_true",
                        help="Include RouteViews validation during validate mode")
    args = parser.parse_args()

    os.makedirs(args.out, exist_ok=True)
    progress_file = os.path.join(args.out, "progress.json")

    progress = {"stage": args.mode, "country": args.country,
                "done": False, "asn_list": args.asn}
    safe_write(progress_file, json.dumps(progress, indent=2))

    if args.mode == "potaroo":
        fetch_asn_data(output_dir=args.out,
                       country=args.country, asn_list=args.asn)
    ...
    progress["done"] = True
    safe_write(progress_file, json.dumps(progress, indent=2))
    print(f"✅ Job completed. Progress saved at {progress_file}")
